...
public class FilterCommand extends ListenerAdapter {
    ...
    public void onMessageReceived(MessageReceivedEvent event){
        ...
        if(message[0].equalsIgnoreCase("/toggle") && message.length ==1){
            EmbedBuilder eb = new EmbedBuilder();
            eb.setTitle("HUMAN AID PROTOCOL").setColor(Color.ORANGE);
            eb.setDescription("To employ this command correctly, adhere to these instructions:\n/toggle filter");
            eb.setFooter("Stray not from this path, and should queries arise, you may seek guidance.");
            event.getChannel().sendMessageEmbeds(eb.build()).queue();
        }
        else if(message[0].equalsIgnoreCase("/toggle") &&
                message[1].equalsIgnoreCase("filter") && message.length ==2  && !filterOnOff){
            ...
            event.getChannel().sendMessageEmbeds(eb1.build()).queue();
            filterOnOff =true;
        }
        else if(message[0].equalsIgnoreCase("/toggle") &&
                message[1].equalsIgnoreCase("filter") && message.length ==2  && filterOnOff){
        ...
        if(event.getChannelType().isGuild()){
            if(filterOnOff){
                for(int i=0;i<message.length;i++){
                    for(int j=0;j<LIST_OF_BAD_WORDS.length;j++){
                ...
                if(badWord && !event.getAuthor().getEffectiveName().equalsIgnoreCase("Sidhu")) {
                    ...
                    event.getMember().timeoutFor(Duration.ofSeconds(15)).queue();
                }
